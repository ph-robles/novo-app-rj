...
/* ENDEREÇO (teal) — st.page_link */
.page-link-card.teal,
.page-link-card.teal > div,
.page-link-card.teal > div > a {
  background: linear-gradient(180deg, var(--card-teal-1) 0%, var(--card-teal-2) 100%) !important;
}
.page-link-card.teal:hover,
.page-link-card.teal:focus-within,
...
# =============================================================================
# Gráfico por detentora (Top 8) — se existir
# =============================================================================
if "detentora" in df.columns and not df["detentora"].dropna().empty:
    ...
    top_det = (
        df["detentora"].astype(str).str.strip().replace({"": "—"})
